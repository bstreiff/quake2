...
void M_DrawTextBox (int x, int y, int width, int lines)
{
   ...
   int		cx, cy;
	...
	M_DrawCharacter (cx, cy+8*scale, 7);
	...
	while (width > 0)
	{
		...
		M_DrawCharacter (cx, cy+8*scale, 8);
		width -= 1;
		cx += 8*scale;
	}
	...
	M_DrawCharacter (cx, cy+8*scale, 9);
}
...
void M_Main_Draw (void)
{
   const float scale = vid_hudscale->value;
	...
	{
		if ( i != m_main_cursor )
			re.DrawPic( xoffset, ystart + i * 40 * scale + 13 * scale, names[i] );
	}
	strcpy( litname, names[m_main_cursor] );
	strcat( litname, "_sel" );
	re.DrawPic( xoffset, ystart + m_main_cursor * 40 * scale + 13 * scale, litname );

	M_DrawCursor( xoffset - 25 * scale, ystart + m_main_cursor * 40 * scale + 11 * scale, (int)(cls.realtime / 100)%NUM_CURSOR_FRAMES );

	re.DrawGetPicSize( &w, &h, "m_main_plaque" );
	re.DrawPic( xoffset - 30 * scale - w, ystart, "m_main_plaque" );

	re.DrawPic( xoffset - 30 * scale - w, ystart + h + 5 * scale, "m_main_logo" );
}
...
void Multiplayer_MenuInit( void )
{
   const float scale = vid_hudscale->value;
	...
	s_player_setup_action.generic.y		= 20 * scale;
...
static void KeyCursorDrawFunc( menuframework_s *menu )
{
   const float scale = vid_hudscale->value;
	if ( bind_grab )
		...
		re.DrawChar( menu->x, menu->y + menu->cursor * 9*scale, 12 + ( ( int ) ( Sys_Milliseconds() / 250 ) & 1 ) );
}
...
static void DrawKeyBindingFunc( void *self )
{
   const float scale = vid_hudscale->value;
	...
	else
	{
		...
		x = strlen(name) * 8*scale;

		if (keys[1] != -1)
		{
			Menu_DrawString( a->generic.x + a->generic.parent->x + 24*scale + x, a->generic.y + a->generic.parent->y, "or" );
			Menu_DrawString( a->generic.x + a->generic.parent->x + 48*scale + x, a->generic.y + a->generic.parent->y, Key_KeynumToString (keys[1]) );
		}
	}
}
...
static void UpdateSoundQualityFunc( void *unused )
{
   const float scale = vid_hudscale->value;
	...
	M_DrawTextBox( 8*scale, (120 - 48)*scale, 36, 3 );
...
void Options_MenuInit( void )
{
	...
	*/
	...
	s_options_menu.y = viddef.height / 2 - 58*scale;
	...
	s_options_sfxvolume_slider.maxvalue		= 10*scale;
	s_options_sfxvolume_slider.curvalue		= Cvar_VariableValue( "s_volume" ) * 10;

	s_options_cdvolume_box.generic.type	= MTYPE_SPINCONTROL;
	s_options_cdvolume_box.generic.x		= 0;
	s_options_cdvolume_box.generic.y		= 10*scale;
	...
	s_options_quality_list.generic.y		= 20*scale;
	...
	s_options_compatibility_list.generic.y		= 30*scale;
	...
	s_options_sensitivity_slider.generic.y		= 50*scale;
	...
	s_options_alwaysrun_box.generic.y	= 60*scale;
	...
	s_options_invertmouse_box.generic.y	= 70*scale;
	...
	s_options_lookspring_box.generic.y	= 80*scale;
	...
	s_options_lookstrafe_box.generic.y	= 90*scale;
	...
	s_options_freelook_box.generic.y	= 100*scale;
	...
	s_options_crosshair_box.generic.y	= 110*scale;
...
/*
	...
	s_options_noalttab_box.generic.y	= 110*scale;
...
*/
	...
	s_options_gamepad_box.generic.y = 120*scale;
	...
	s_options_customize_options_action.generic.y		= 140*scale;
	s_options_customize_options_action.generic.name	= "customize controls";
	s_options_customize_options_action.generic.callback = CustomizeControlsFunc;

	s_options_defaults_action.generic.type	= MTYPE_ACTION;
	s_options_defaults_action.generic.x		= 0;
	s_options_defaults_action.generic.y		= 150*scale;
	s_options_defaults_action.generic.name	= "reset defaults";
	s_options_defaults_action.generic.callback = ControlsResetDefaultsFunc;

	s_options_console_action.generic.type	= MTYPE_ACTION;
	s_options_console_action.generic.x		= 0;
	s_options_console_action.generic.y		= 160*scale;
...
void M_Credits_MenuDraw( void )
{
	...
	** draw the credits
	*/
	for ( i = 0, y = viddef.height - ( ( cls.realtime - credits_start_time ) / 40.0F ); credits[i] && y < viddef.height; y += 10*scale, i++ )
	{
		...
		for ( j = 0; credits[i][j+stringoffset]; j++ )
		{
			...
			x = ( viddef.width - strlen( credits[i] ) * 8*scale - stringoffset * 8*scale ) / 2 + ( j + stringoffset ) * 8*scale;
...
void Game_MenuInit( void )
{
   const float scale = vid_hudscale->value;
	...
	s_medium_game_action.generic.y		= 10*scale;
	...
	s_hard_game_action.generic.y		= 20*scale;
	...
	s_load_game_action.generic.y		= 40*scale;
	...
	s_save_game_action.generic.y		= 50*scale;
...
void LoadGame_MenuInit( void )
{
   const float scale = vid_hudscale->value;
	...
	s_loadgame_menu.y = viddef.height / 2 - 58*scale;
	...
	for ( i = 0; i < MAX_SAVEGAMES; i++ )
	{
		...
		s_loadgame_actions[i].generic.y = ( i ) * 10 * scale;
		if (i>0)	// separate from autosave
			s_loadgame_actions[i].generic.y += 10 * scale;
...
void SaveGame_MenuInit( void )
{
   const float scale = vid_hudscale->value;
	...
	s_savegame_menu.y = viddef.height / 2 - 58*scale;
	...
	for ( i = 0; i < MAX_SAVEGAMES-1; i++ )
	{
		...
		s_savegame_actions[i].generic.y = ( i ) * 10*scale;
...
void JoinServer_MenuInit( void )
{
   ...
   int i;
	...
	s_joinserver_search_action.generic.y	= 10*scale;
	...
	for ( i = 0; i < MAX_LOCAL_SERVERS; i++ )
	{
		...
		s_joinserver_server_actions[i].generic.y		= (40 + i*10)*scale;
...
//=======
	...
	*/
	...
	s_rules_box.generic.y	= 20*scale;
...
//PGM
	...
	s_timelimit_field.generic.y	= 36*scale;
	...
	s_fraglimit_field.generic.y	= 54*scale;
	...
	*/
	...
	s_maxclients_field.generic.y	= 72*scale;
	s_maxclients_field.generic.statusbar = NULL;
	...
	s_hostname_field.generic.y	= 90*scale;
	...
	s_startserver_dmoptions_action.generic.y	= 108*scale;
	s_startserver_dmoptions_action.generic.statusbar = NULL;
	...
	s_startserver_start_action.generic.y	= 128*scale;
...
//ROGUE
	if(Developer_searchpath(2) == 2)
	{
		...
		s_no_mines_box.generic.y	= y += 10*scale;
		...
		s_no_nukes_box.generic.y	= y += 10*scale;
		...
		s_stack_double_box.generic.y	= y += 10*scale;
		...
		s_no_spheres_box.generic.y	= y += 10*scale;
...
void AddressBook_MenuInit( void )
{
   const float scale = vid_hudscale->value;
	...
	for ( i = 0; i < NUM_ADDRESSBOOK_ENTRIES; i++ )
	{
		...
		s_addressbook_fields[i].generic.y		= i * 18 * scale;
...
void PlayerConfig_MenuDraw( void )
{
   const float scale = vid_hudscale->value;
	...
	if ( s_pmi[s_player_model_box.curvalue].skindisplaynames )
	{
		...
		M_DrawTextBox( ( refdef.x ) * ( 320.0F / viddef.width ) - 8, ( viddef.height / 2 ) * ( 240.0F / viddef.height) - 77*scale, refdef.width / 8, refdef.height / 8 );
		refdef.height += 4;
		...
		re.DrawPic( s_player_config_menu.x - 40*scale, refdef.y, scratch );
	}
}
